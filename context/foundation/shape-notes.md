...
- **Regular user** (default) — can track their own dog's trick progress, follow other users (one-way, no friend request needed), and see followed users' progress in a feed
- Following is one-way (like Twitter/Instagram) — User A can follow User B without reciprocation

The smallest MVP access model: regular users track progress and can optionally follow others to see their progress. Admin role exists to curate the trick catalog, keeping it high-quality and domain-focused.
